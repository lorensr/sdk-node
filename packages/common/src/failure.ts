...
export type TimeoutType = coresdk.common.TimeoutType;
...
export class TemporalFailure extends Error {
   ...
   */
  public failure?: coresdk.failures.IFailure;
...
): Promise<coresdk.failures.IFailure | undefined> {
 ...
 */
export async function errorToFailure(err: unknown, dataConverter: DataConverter): Promise<coresdk.failures.IFailure> {
    ...
    if (err instanceof ApplicationFailure) {
      return {
        ...
        applicationFailureInfo: {
          ...
          details: err.details ? await dataConverter.toPayloads(...err.details) : undefined,
        },
      };
    }
    ...
    if (err instanceof TimeoutFailure) {
      return {
        ...
        timeoutFailureInfo: {
          ...
          lastHeartbeatDetails: await dataConverter.toPayloads(err.lastHeartbeatDetails),
        },
      };
    }
...
export async function optionalFailureToOptionalError(
  failure: coresdk.failures.IFailure | undefined | null,
  dataConverter: DataConverter
...
export async function failureToErrorInner(
  failure: coresdk.failures.IFailure,
  dataConverter: DataConverter
  ...
  if (failure.applicationFailureInfo) {
    return new ApplicationFailure(
      ...
      await arrayFromPayloads(dataConverter, failure.applicationFailureInfo.details),
  ...
  if (failure.timeoutFailureInfo) {
    return new TimeoutFailure(
      failure.message ?? undefined,
      await dataConverter.fromPayloads(0, failure.timeoutFailureInfo.lastHeartbeatDetails),
      failure.timeoutFailureInfo.timeoutType ?? coresdk.common.TimeoutType.TIMEOUT_TYPE_UNSPECIFIED
    );
  }
  ...
  if (failure.cancelledFailureInfo) {
    return new CancelledFailure(
      failure.message ?? undefined,
      await arrayFromPayloads(dataConverter, failure.cancelledFailureInfo.details),
      await optionalFailureToOptionalError(failure.cause, dataConverter)
    );
  }
  ...
  if (failure.activityFailureInfo) {
    if (!failure.activityFailureInfo.activityType) {
      throw new TypeError('Missing activityType on activityFailureInfo');
    }
    return new ActivityFailure(
      failure.activityFailureInfo.activityType,
      failure.activityFailureInfo.activityId ?? undefined,
      failure.activityFailureInfo.retryState ?? coresdk.common.RetryState.RETRY_STATE_UNSPECIFIED,
